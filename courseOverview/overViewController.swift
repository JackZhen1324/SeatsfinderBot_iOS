...
class overViewController: UITableViewController {
   ...
   func setupButton()
   {
        ...
        if modeInput_.selectedSegmentIndex == 0 {
            ...
            if (self.studetnID_ ?? "").isEmpty   || (self.studentPas ?? "").isEmpty
            {
                ...
                else
                {
                    ...
                    print("we are here1")
                    SendSchdeuleRequest(url: url, project: project, spider: spider, semester: semester, section: section, level: level, choice: choice, reserved: reserved, username: username, password: password, timeinterval: "2", deviceId: newToken)
                }
            }
            else
            {
    ...
    func SendSchdeuleRequest(url:String?,project:String?,spider:String?,semester:String?,section:String?,level:String?,choice:String?,reserved:String?,username:String?,password:String?,timeinterval:String?,deviceId:String?){
        var timeInterval:String! = "2"
        var data = ["project":project!,"spider":spider!,"semester":semester! ,"section":section!,"level":level!,"choice":choice!,"reserved":reserved!,"username":username!,"password":password!,"timeInterval":timeInterval!,"deviceID":timeinterval!]
